...
def leaf_recognision(file_path):
    execution_path = os.getcwd()
    detector = ObjectDetection()
    detector.setModelTypeAsRetinaNet()
    detector.setModelPath( os.path.join(execution_path , "resnet50_coco_best_v2.1.0.h5"))
    detector.loadModel()
    detections = detector.detectObjectsFromImage(input_image=os.path.join(execution_path , file_path), output_image_path=os.path.join(execution_path , file_path))

    return True if detections == "Leaf" else False
...
def upload():
    ...
    if request.method == 'POST':
        ...
        leaf_check = leaf_recognision(file_path)
        ...
        if leaf_check == True:
            ...
            return "Given Image is not Leaf"
